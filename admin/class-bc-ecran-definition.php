...
class BC_Ecran_Definition {
	...
	private function parcours( $post, $definition ) {
		...
		$prefere  = BC_Gabarits::chemin_prefere( $definition );
		$dossier  = dirname( $prefere );
		$ecrivant = is_dir( $dossier ) ? is_writable( $dossier ) : is_writable( dirname( $dossier ) );
			...
			array(
				...
				'quoi'  => '' !== trim( (string) $post->post_title )
					? sprintf(
						...
						'<code>' . esc_html( BC_Definition::nom( $definition ) ) . '</code>'
					)
		...
		if ( $nouveau ) {
			$etapes[] = array(
				...
				'quoi'  => sprintf(
					...
					'<code>' . esc_html( BC_Gabarits::chemin_court( $prefere ) ) . '</code>'
				),
			);
		} elseif ( '' !== $gabarit ) {
			$etapes[] = array(
				...
				'quoi'  => sprintf(
					...
					'<code>' . esc_html( BC_Gabarits::chemin_court( $gabarit ) ) . '</code>'
				),
			);
		} else {
			$etapes[] = array(
				...
				'quoi'  => $ecrivant
					...
					: sprintf(
						...
						'<code>' . esc_html( BC_Gabarits::chemin_court( $dossier ) ) . '</code>'
					),
			);
		}
	...
	public function metabox_fiche( $post ) {
		$definition = BC_Definition::charger( $post ) ?? BC_Definition::vierge();
